...
class string_extend {
    ...
    similarity_edit_dist(v_s_: string, v2_s_: string): number {
        let v_len_n = v_s_.length;
        let v2_len_n = v2_s_.length;
        // 安检
        {
            if (v_len_n == 0) {
                return v2_len_n;
    ...
    fuzzy_match(args_: string | string[], key_s_: string, min_simile_n_ = 0): string | null {
        ...
        let match_result_ss: string[] = [];
        let key_ss = key_s_.split('');
        let index_n: number;
        ...
        // 返回相识度最高的字符串
            ...
            match_result_ss.sort(
                (v_a_s: string, v_b_s: string) =>
                    this.similarity_edit_dist(v_b_s, key_s_) - this.similarity_edit_dist(v_a_s, key_s_)
            );
